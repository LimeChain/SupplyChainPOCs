...
import (
	...
	chaincode "github.com/LimeChain/SupplyChainPOCs"
...
var _ = Describe("Tests for SupplyChainChaincode", func() {
	...
	stub := shim.NewMockStub("testingStub", new(chaincode.SupplyChainChaincode))
	var result peer.Response

	BeforeSuite(func() {
		stub.MockInit("000", [][]byte{
			[]byte(constants.Init),
			[]byte(constants.ORG_ONE),
			[]byte(constants.ORG_TWO),
			[]byte(constants.ORG_THREE)})
	})
