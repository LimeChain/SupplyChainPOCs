		...
		Describe("manufacture functionality", func() {
			...
			It("Should successfully execute manufacture", func() {
				...
				recordDto = poc01_1.CertifiedRecordDto{
					...
					AssembableRecordDto: &dto.AssembableRecordDto{
						AssembledFrom: dto.RecordPartsDto{
							{
				...
				for index, _ := range payload.AssembledFrom {
					Expect(payload.AssembledFrom[index]).To(Equal(recordDto.AssembledFrom[index]))
				}
			...
			It("Should unsuccessfully execute manufacture due to invalid AssetId", func() {
				recordDto = poc01_1.CertifiedRecordDto{
					...
					QualityCertificates: []string{
						constants.ExampleTest, constants.Manufacture,
					},
				}
